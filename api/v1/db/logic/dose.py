...
class DoseHelper(Helper):
    ...
    def get_doses(cls, dict_url: dict) -> Optional[list[Dose]]:
        ...
        if "dropper_id" in dict_url:
            query["$match"].update(
                {"doses.dropper_id": dict_url["dropper_id"]})
        ...
        if "application_datetime" in dict_url:
            query["$match"].update(
                {"doses.application_datetime": dict_url["application_datetime"]})
        ...
        if "end" in dict_url or "start" in dict_url:
            ...
            if "start" in dict_url:
                query["$match"]["doses.application_datetime"].update(
                    {"$gte": dict_url["start"]})
            if "end" in dict_url:
                query["$match"]["doses.application_datetime"].update(
                    {"$lte": dict_url["end"]})
        doses_db = db_client.droppers.aggregate(
